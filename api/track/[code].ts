...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const userResult = await p.query('SELECT id FROM ref_users WHERE referral_code = $1 AND deleted_at IS NULL', [code]);
      ...
      if (userResult.rows.length > 0) {
        ...
        if (fullName && whatsappNumber) {
          const leadResult = await p.query(
      ...
      const trackResult = await p.query('SELECT id FROM ref_tracking_links WHERE code = $1 AND is_active = true', [code]);
  ...
  if (req.method === 'GET') {
    try {
      ...
      const userResult = await p.query('SELECT id, first_name, last_name, profile_image_url, referral_code FROM ref_users WHERE referral_code = $1 AND deleted_at IS NULL', [code]);
      if (userResult.rows.length > 0) {
        return res.status(200).json({ type: 'ambassador', data: userResult.rows[0] });
      }
      const trackResult = await p.query('SELECT id, name, platform, code FROM ref_tracking_links WHERE code = $1 AND is_active = true', [code]);
      if (trackResult.rows.length > 0) {
